...
int isLessOrEqual(int x, int y) {
    int dif = y + (~x) + 1;
    int rsum = y + x; //
    int res = !(dif >> 31); //1 if positive (x<=y)
    int sdif = ((x ^ y) >> 31) & 1; //different signs?
    int over = (sdif & ((x ^ rsum) >> 31));
    int xsign = (x >> 31) & 1; //1 if negative
    return ((res & ~over) & !sdif) | (xsign & sdif);
}
/*
