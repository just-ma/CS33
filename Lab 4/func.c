...
#pragma omp parallel for firstprivate(n, arrayX, arrayY, seed) private(i)
  for(i = 0; i < n; i++){
  ...
  omp_set_num_threads(num_threads);
#pragma omp parallel for firstprivate(n, arrayX, arrayY, array, Y, Z, Ones, iter, max_size, index, objxy, probability) private(i, j, index_X, index_Y)
  for(i = 0; i<n; i++){
    for(j = 0; j < Ones; j++){
    ...
    probability[i] = probability[i]/yes;
  }
}
...
{
  int i, j;
  double allWeights = 1/((double)(n));
    
  omp_set_num_threads(num_threads);
#pragma omp parallel for firstprivate(n, arrayX, arrayY, x_j, y_j, cfd, u) private(j, i)
  for(j = 0; j < n; j++){
  ...
  omp_set_num_threads(num_threads);
#pragma omp parallel for firstprivate(n, arrayX, arrayY, x_j, y_j, weights, allWeights) private(i)
  for(i = 0; i < n; i++){
    arrayX[i] = x_j[i];
    arrayY[i] = y_j[i];
    weights[i] = allWeights;
  }
}
